...
def build_diagnostics_snapshot(*, memory_items: int, voice_muted: bool, wake_word: str) -> DiagnosticsSnapshot:
    data_dir = _project_root() / "atlas_data"
    files = [
        data_dir / "calendar.json",
        data_dir / "notes.json",
        data_dir / "email_outbox.json",
        data_dir / "smart_home.json",
    ]
    files_ok = data_dir.exists() and all(f.exists() for f in files)
